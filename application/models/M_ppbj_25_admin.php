<?php
class M_ppbj_25_admin extends CI_Model
{
    ...
    public function cetak_ppbj_25($id_bln)
    {


        $ta = $this->session->userdata('ta');
        $this->datatables->select('*');
        $this->datatables->join('tbl_unit', 'tbl_unit.id_unit = tbl_ppbj_25.id_unit', 'left');
        //$this->datatables->join('stts_apbd', 'tbl_apbd.id_bln = stts_apbd.id_bln', 'left');
        $this->datatables->where('tbl_ppbj_25.id_bln', $id_bln);
        $this->datatables->where('tbl_ppbj_25.tahun', $ta);
        $this->db->order_by('tbl_unit.urut','asc');
        $this->datatables->from('tbl_ppbj_25');
        return $this->db->get('tbl_ppbj_25')->result();
    }
